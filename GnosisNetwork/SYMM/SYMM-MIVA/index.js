...
const GNO_PER_WEEK = bnum(104.78205);
const BLOCKS_PER_SNAPSHOT = 679; // 256;

const NUM_SNAPSHOTS = Math.ceil(
    (END_BLOCK - START_BLOCK) / BLOCKS_PER_SNAPSHOT
);
const BAL_PER_SNAPSHOT = GNO_PER_WEEK.div(bnum(NUM_SNAPSHOTS)); // Ceiling because it includes end block
...
(async function () {
    ...
    let pools = await fetchAllPools(web3.utils, END_BLOCK);
     console.log(`XXXX pools--->>>>>>>> ${JSON.stringify(pools)}`);
