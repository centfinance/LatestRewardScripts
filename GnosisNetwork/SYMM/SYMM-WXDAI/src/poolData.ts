...
function atLeastTwoTokensHavePrice(tokens, prices): boolean {
    ...
    for (const token of tokens) {
        console.log(
            `TOKEN TOKEN : ${token.tokenId} -- ${token.tokenId}`
        );
...
async function tokenMetrics(
    ...
    for (const token of tokens) {
        ...
        console.log(`bTokenDecimals: ${bTokenDecimals}`);
        ...
        console.log(`origLiq: ${origLiquidity}`);
        tokenData.push({
...
export async function getPoolInvariantData(
    ...
    const mappedlist = eval(addressMaps.xdaiEthMap);
